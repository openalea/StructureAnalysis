...
#include "distribution_reestimation.hpp"
...
MultivariateMixtureData* MultivariateMixture::cluster(StatError &error,  const Vectors &vec,
  ...
  stat_tool::variable_nature *itypes = NULL; // type des variables
  ...
  if (states != NULL) {
    ...
    state_vec = new Vectors(nb_vector, vec.identifier, nb_res_variable, itypes,			    iint_vector, ireal_vector);
