...
/* RWspace Histogram::binaryStoreSize() const

{
  RWspace size = sizeof(nb_value) + sizeof(offset) + sizeof(nb_element) +
                 sizeof(max) + sizeof(mean) + sizeof(variance) +
                 sizeof(*frequency) * nb_value;

  return size;
}


void Histogram::restoreGuts(RWvistream &is)

{
  ...
  is >> nb_value >> offset >> nb_element >> max >> mean >> variance;
  alloc_nb_value = nb_value;
...
void Histogram::restoreGuts(RWFile &file)

{
  ...
  frequency = new int[nb_value];
  file.Read(frequency , nb_value);
}


void Histogram::saveGuts(RWvostream &os) const

{
  ...
  os << nb_value << offset << nb_element << max << mean << variance;
...
void Histogram::saveGuts(RWFile &file) const
