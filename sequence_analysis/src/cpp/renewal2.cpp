 ...
 *
 *  arguments : stream, pointeur sur un objet Renewal_data,
 *              flag niveau de detail, flag fichier.
 *
 *--------------------------------------------------------------*/

ostream& Renewal::ascii_write(ostream &os , const Renewal_data *timev ,
...

{
  ...
  if ((timev) && (timev->within) && (timev->backward) && (timev->forward)) {
    ...
    if (exhaustive) {
      ...
      // ecriture de l'histogramme des intervalles de temps a l'interieur de la periode d'observation
      ...
      os << "   | " << SEQ_label[SEQL_BACKWARD] << " " << SEQ_label[SEQL_RECURRENCE_TIME]
         << " " << STAT_label[STATL_HISTOGRAM] << " | " << SEQ_label[SEQL_BACKWARD]
  ...
  if (time->variance > 0.) {
    ...
    if (timev) {
      ...
      if (exhaustive) {
        ...
        os << "   | " << SEQ_label[SEQL_OBSERVATION_TIME] << " " << STAT_label[STATL_HISTOGRAM] << endl;
...
bool Renewal::ascii_write(Format_error &error , const char *path ,
...
ostream& Renewal::spreadsheet_write(ostream &os , const Renewal_data *timev) const
...

{
  ...
  if (time->variance > 0.) {
    ...
    if (timev) {
      os << "\n" << SEQ_label[SEQL_OBSERVATION_TIME] << " " << STAT_label[STATL_HISTOGRAM] << "\t";
...
bool Renewal::spreadsheet_write(Format_error &error , const char *path) const
 ...
 *              pointeur sur un objet Renewal_data.
 *
 *--------------------------------------------------------------*/
      ...
      if ((timev) && (timev->within) && (timev->backward) && (timev->forward)) {
        ...
        if (timev->inter_event) {
          out_file << "plot [0:" << inter_event->nb_value - 1 << "] [0:"
                   ...
                   << " title \"" << SEQ_label[SEQL_INTER_EVENT] << " " << STAT_label[STATL_HISTOGRAM]
                   << "\" with impulses,\\" << endl;
        ...
        if (timev->within->nb_element > 0) {
          out_file << "plot [0:" << inter_event->nb_value - 1 << "] [0:"
                   ...
                   << "] \"" << label((data_file_name[0].str()).c_str()) << "\" using " << j++
                   << " title \"" << SEQ_label[SEQL_OBSERVATION_INTER_EVENT] << " " << STAT_label[STATL_HISTOGRAM]
                   << "\" with impulses,\\" << endl;
          out_file << "\"" << label((data_file_name[0].str()).c_str()) << "\" using " << k++
                   << " title \"" << SEQ_label[SEQL_INTER_EVENT] << " " << STAT_label[STATL_DISTRIBUTION];
          inter_event->plot_title_print(out_file);
        ...
        if (timev->length_bias) {
          ...
          out_file << "plot [0:" << inter_event->nb_value - 1 << "] [0:"
                   ...
                   << "] \"" << label((data_file_name[0].str()).c_str()) << "\" using " << j++
                   << " title \"" << SEQ_label[SEQL_LENGTH_BIASED] << " " << STAT_label[STATL_HISTOGRAM]
                   << "\" with impulses,\\" << endl;
          out_file << "\"" << label((data_file_name[0].str()).c_str()) << "\" using " << k++
          ...
          if (time->mass[m] > 0.) {
            ...
            if (timev) {
              out_file << "plot [0:" << nb_event[m]->nb_value - 1 << "] [0:"
                       ...
                       << SEQ_label[SEQL_TIME_UNIT] << " " << STAT_label[STATL_HISTOGRAM] << "\" with impulses,\\" << endl;
            }
...
bool Renewal::plot_write(Format_error &error , const char *prefix ,
...
/* RWDEFINE_COLLECTABLE(Renewal , STATI_RENEWAL);


RWspace Renewal::binaryStoreSize() const

{
  ...
  size = sizeof(type) + sizeof(nb_event_max) + time->binaryStoreSize() +
         inter_event->binaryStoreSize() + length_bias->binaryStoreSize() +
         backward->binaryStoreSize(backward->alloc_nb_value) + forward->binaryStoreSize();

  size += sizeof(true) * (mixture->nb_value - 1);
  for (i = 1;i < mixture->nb_value;i++) {
    if (nevent_time[i]) {
      size += nevent_time[i]->binaryStoreSize();
    }
  }

  for (i = time->offset;i < time->nb_value;i++) {
    if (time->mass[i] > 0.) {
      size += nb_event[i]->binaryStoreSize();
    }
  }

  size += mixture->binaryStoreSize();

  size += index_event->binaryStoreSize();

  if (renewal_data) {
    size += renewal_data->recursiveStoreSize();
  }
...
void Renewal::restoreGuts(RWvistream &is)

{
  ...
  is >> type >> nb_event_max;

  time = new Distribution();
  time->restoreGuts(is);

  inter_event = new Parametric();
  inter_event->restoreGuts(is);
  length_bias = new Length_bias();
  ...
  for (i = 1;i <= nb_event_max;i++) {
    is >> status;
    if (status) {
      nevent_time[i] = new Parametric();
      nevent_time[i]->restoreGuts(is);
    }
    else {
      nevent_time[i] = 0;
    }
  ...
  for (i = time->offset;i < time->nb_value;i++) {
    if (time->mass[i] > 0.) {
      nb_event[i] = new Nb_event();
      nb_event[i]->restoreGuts(is);
    }
    else {
      nb_event[i] = 0;
    }
  ...
  is >> renewal_data;
  if (renewal_data == RWnilCollectable) {
    renewal_data = 0;
  }
}


void Renewal::restoreGuts(RWFile &file)

{
  ...
  file.Read(type);
  file.Read(nb_event_max);

  time = new Distribution();
  time->restoreGuts(file);
  ...
  for (i = 1;i <= nb_event_max;i++) {
    ...
    if (status) {
      nevent_time[i] = new Parametric();
      nevent_time[i]->restoreGuts(file);
    }
    else {
      nevent_time[i] = 0;
    }
...
void Renewal::saveGuts(RWvostream &os) const

{
  register int i;


  os << type << mixture->nb_value - 1;

  time->saveGuts(os);

  inter_event->saveGuts(os);
  length_bias->saveGuts(os);
  backward->saveGuts(os , backward->alloc_nb_value);
  forward->saveGuts(os);

  for (i = 1;i < mixture->nb_value;i++) {
    if (nevent_time[i]) {
      os << true;
      nevent_time[i]->saveGuts(os);
    }
    else {
      os << false;
    }
...
void Renewal::saveGuts(RWFile &file) const

{
  ...
  file.Write(type);
  file.Write(mixture->nb_value - 1);

  time->saveGuts(file);

  inter_event->saveGuts(file);
  length_bias->saveGuts(file);
  backward->saveGuts(file , backward->alloc_nb_value);
  forward->saveGuts(file);

  for (i = 1;i < mixture->nb_value;i++) {
    if (nevent_time[i]) {
      file.Write(true);
      nevent_time[i]->saveGuts(file);
    }
    else {
      file.Write(false);
    }
