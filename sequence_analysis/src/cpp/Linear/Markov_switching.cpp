 ...
 * Constructeur de la classe Markov_switching
 *
 * arguments : nombre d'états, nombre de processus d'observation,
 *             nombre de valeurs observées par processus
 *
 *--------------------------------------------------------------*/

Markov_switching::Markov_switching(int inb_state ,  int *nb_value)
:Chain('o' , inb_state , (int)pow((double)inb_state , 1) , true)
{
  register int i;
  ...
  sw_process = new Switching_process*[2];
  sw_process[0] = 0;
  sw_process[1] = new Switching_process(nb_state , *nb_value++);

  nonparametric_process = new Nonparametric_sequence_process*[2];
 ...
 *              sur un objet Switching_process, longueur des sequences,
 *
 *--------------------------------------------------------------*/

Markov_switching::Markov_switching(const Chain *pchain ,
 ...
 *
 * arguments : pointeur sur un objet Markov_switching
 *
 *--------------------------------------------------------------*/

Markov_switching::Markov_switching(const Markov_switching &isw_markov)
  :Chain('o' , isw_markov.nb_state , (int)pow((double)isw_markov.nb_state , 1),   true)
{
  register int i , j;
  ...
  sw_process = new Switching_process*[2];
  sw_process[0] = 0;
  sw_process[1] = new Switching_process(*(isw_markov.sw_process[1]) , 'c' , false);
...
void Markov_switching::copy(const Markov_switching &isw_markov , bool data_flag, int param)
{
  ...
  nonparametric_process[1] = 0;
  sw_process[1] = new Switching_process(*(isw_markov.sw_process[1]) , 'c');
 ...
 *
 * arguments : reference sur un objet Markov_switching
 *
 *--------------------------------------------------------------*/
 ...
 *
 * arguments : référence sur un objet Format_error
 *
 *--------------------------------------------------------------*/
...
Markov_switching_data* Markov_switching::extract_data(Format_error &error) const
{
  ...
  else if (sw_markov_data->nb_variable != 2) {
    status = false;
...
/*--------------------------------------------------------------*
 *
 *  Application d'un seuil sur les parametres d'une (semi-)chaine 
 *  de Markov_switching.
 *
 ...
 *
 *  arguments : stream, pointeur sur un objet Markov_switching_data,
                             ...
                             bool file_flag ) const
{
  ...
  Continuous_histo **observation = 0;
  ...
  os << "\n" << 1 << " "
     << STAT_word[STATW_OUTPUT_PROCESS] << endl;


  // ecriture des lois associees au processus d'observation
  ...
  sw_process[1]->ascii_print(os , observation , exhaustive , file_flag); 
					     ...
					     bool hidden) const
{
 ...
 // ecriture des lois d'occupation des etats
  ...
  sw_process[1]->spreadsheet_print(os , observation);
...
/*--------------------------------------------------------------*
 *
 *  Calcul du nombre de parametres independants d'un objet Markov_switching.
 *
 *  argument : probabilite minimum.
 *
 *--------------------------------------------------------------*/
...
int Markov_switching::nb_parameter_computation(double min_probability) const
{
  ...
  nb_parameter += sw_process[1]->nb_parameter_computation();
...
/*--------------------------------------------------------------*
 *
 *  Calcul des lois caracteristiques d'un objet Markov_switching.
 *
