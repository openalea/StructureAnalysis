...
# endif
	...
	for (j = 0 ; j < hsw_markov->nb_output_process; j++) {
	  ...
	  for (i = 0; i<nb_covariable; i++){
	    for (m = 0; m < nb_covariable; m++){
	      cout<<gsl_matrix_get(temp_add,i,m)<<" ";
	    }
	    cout<<endl;
	  }
	  cout<<endl<<endl;
...
# endif
	...
	for (k = 0; k < hsw_markov->nb_state; k++) {
	  ...
	  for (cat = 0; cat < (nb_cat-1); cat++){
		...
		for (m = 0; m < length[i]; m++){
		  gsl_matrix_set(W_icatk, m, m, hsw_markov->sw_process[j+1]->observation[k]->
				 variance_computation_multi(int_sequence[i][j][m],covar[i][m]));
		  gsl_vector_set(mu_icatk, m, hsw_markov->sw_process[j+1]->observation[k]->
				 mean_computation_multi(int_sequence[i][j][m], covar[i][m]));

		  if (int_sequence[i][j][m] == cat){
		    gsl_vector_set(delta, m, 1.);
		  }
		  else {
		    gsl_vector_set(delta, m, 0.);
		  }
		}

		gsl_vector_sub(delta, mu_icatk); // delta = delta - mu_icatk
		...
		//somme des tXLWX
		gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1., X_i, L_ik, 0., temp_mult);
		gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1., temp_mult, W_icatk, 0., temp_mult1);
		gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1., temp_mult1, X_i, 0., temp_mult2);
		gsl_matrix_add(temp_add, temp_mult2); 

		//somme des tXL(delta-mu)
		gsl_blas_dgemv(CblasNoTrans, 1., temp_mult, delta, 0., temp_mult3);
		gsl_vector_add(add_vect, temp_mult3);  
