...
import cstat_tools, trees, chmt
inf_bound=1
sup_bound=3
probability= 0.6
ident=cstat_tools.DistributionIdentifier.UNIFORM
parameter=cstat_tools.D_DEFAULT()
distrib= cstat_tools.Parametric(ident, inf_bound, sup_bound, parameter, 
                                probability)
...
EH=T.EstimationCiHmot(H, True, cstat_tools.VITERBI, 20, True)
print EH.Display(False)
print "\nEstimated HMT (initialization by self-transition): \n "
EH=T.EstimationCiHmot(2, False, False, cstat_tools.VITERBI, 0.9999, 10)
print EH.Display(False)
