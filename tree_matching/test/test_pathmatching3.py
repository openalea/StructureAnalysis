...
class MyMatching(GeneralMatchPath):
    def __init__(self,maxinrange,maxoutrange,deg):
        inconnect = [list(range(maxinrange+min(maxoutrange,max(0,i-deg/2)),maxinrange+min(maxoutrange,1+i+deg/2))) for i in range(maxinrange)]
        outconnect =  [list(range(min(maxinrange,max(0,i-deg/2)), min(maxinrange,1+i+deg/2))) for i in range(maxoutrange)]
