...
TreeMatch_U::TreeMatch_U( MTG& mtg, Array* roots)
{
  _mtg=&mtg;
  assert(roots);
...
/* TreeMatching Topologique non ordonné */

TreeMatch_U::TreeMatch_U(MTG& mtg,
			 Array* roots,
			 ...
			 double coeff)
{
 ...
 //cerr<<"treematching topologique "<<endl;
  ...
  if (!strcmp(mapping,"General"))
       _mapping = GENERAL;
  if (!strcmp(mapping,"MinimumConnectedComponents"))
      _mapping = MINIMUMCONNECTEDCOMPONENTS;
...
//------------------------------------------------------------------------------------------------------
void TreeMatch_U::topologicalMatching()
{
...
// WEIGHTED MATCHING
//------------------------------------------------------------------------------------------------------
void TreeMatch_U::weightedMatching()
{
...
//--------------------------------------------------------------------------------------
DistanceType TreeMatch_U::MatchByTopology(TreeGraph& Tree1,
					  TreeGraph& Tree2,
					  Sequence * S)
{
  ...
  if (_scaleType == SINGLESCALE){
	...
	if (_mapping == GENERAL){
	  ...
	  if (_mapping == MINIMUMCONNECTEDCOMPONENTS){
	    //	  cerr<<"Minimum Connected Components"<<endl;
	    M = new Matching_U_MinCo(Tree1,Tree2,*MCF);
	  }
	  else {if (_mapping == ENDSPACEFREE){
	    M = new EndSpaceFreeMatching_U(Tree1,Tree2,*MCF);
	    //EndSpaceFreeMatching_U* M = new EndSpaceFreeMatching_U(Tree1,Tree2,*MCF);
	    //cout<<M->getI_v()<<endl; 
	    //cout<<M->getR_v()<<endl;
	  }
	  else{
	    cerr<<"Not yet Implemented (1)"<<endl<<"General Edit Matching"<<endl;
	    M = new Matching_U(Tree1,Tree2,*MCF);
	  }
	  }
	}
      ...
      else 
	{
	  if (_mappingType == LOCAL){
	    if (_mapping == GENERAL){
			M = new LocalMatching_U(Tree1,Tree2,*MCF);
			
	    }
	    else{
	      cerr<<"Not yet Implemented "<<endl<<"Local topological Matching"<<endl;
	      M = new LocalMatching_U(Tree1,Tree2,*MCF);
	    }
	  }
	}
      ...
      if (_matchingType == LARGESTCOMMONSUBTREE){
	M = new Matching_U_LargestCommonSubTree(Tree1,Tree2,*MCF);
      }
      else {
	cerr<<"Not yet Implemented"<<endl<<"Largest Common SubTree Matching"<<endl;
	M = new Matching_U_LargestCommonSubTree(Tree1,Tree2,*MCF);
      }
    }
    ...
    M = new MultiscaleMatching_U(Tree1,Tree2,*MCF);
  }
...
// STANDARTIZATION
//---------------------------------------
void TreeMatch_U::standardization()
{
...
DistanceType TreeMatch_U::meanAbsoluteDifferenceComputation(int variable)
{
  int nb_tree = _distances.size();
