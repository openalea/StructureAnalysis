...
TreeMatch_O::TreeMatch_O( MTG& mtg, Array* roots)
{
  _mtg=&mtg;
  assert(roots);
...
TreeMatch_O::TreeMatch_O(MTG& mtg,
			 Array* roots,
			 ...
			 double coeff)
{
 ...
 _InsDelCostCoeff =coeff;
  ...
  if (!strcmp(scale_type,"MultiScale"))
    _scaleType = MULTISCALE;
  if (!strcmp(scale_type,"Quotiented"))
    _scaleType = QUOTIENTED;
...
//------------------------------------------------------------------------------------------------------
void TreeMatch_O::topologicalMatching()
{
...
// WEIGHTED MATCHING
//------------------------------------------------------------------------------------------------------
void TreeMatch_O::weightedMatching()
{
...
//--------------------------------------------------------------------------------------
DistanceType TreeMatch_O::MatchByTopology(TreeGraph& Tree1,
					  TreeGraph& Tree2,
					  Sequence * S)
{
  NodeCost *MCF;
  ...
  if (_localFun){
    ...
    {if (_mapping == SIMILARITY)
      MCF=new NodeCost(SCORE);
    else
      MCF=new NodeCost(TOPOLOGIC);
    }
  DistanceType D;
  if (_scaleType == SINGLESCALE){
    //cout<<"singlescale"<<endl;
    if (_matchingType == EDITION){
      if (_mappingType == GLOBAL){
	if (_mapping == GENERAL){
	  //cout<<"general"<<endl;
	  M = new Matching_O(Tree1,Tree2,*MCF);
	}
	else if (_mapping == DISTANCE){
	  //cout<<"distance"<<endl;
	  M = new Matching_O(Tree1,Tree2,*MCF);
	}
	else if (_mapping == SIMILARITY){
	  //cout<<"similarity"<<endl;
	  M = new MatchingGosse(Tree1,Tree2,*MCF);
	}
	else if (_mapping == LCA){
	  //cout<<"lca"<<endl;
	  M = new MatchingGosdeLca(Tree1,Tree2,*MCF);
	}
	else{
      ...
      else if (_mappingType == LOCAL){
	if (_mapping == SIMILARITY){ 
	  M = new MatchingLosse(Tree1,Tree2,*MCF);
	}
	else{
	  cerr<<"Not yet Implemented"<<endl<<"General topological Matching"<<endl;
	  M = new Matching_O(Tree1,Tree2,*MCF);
    ...
    if (_scaleType == MULTISCALE){
      //cout<<"multiscale"<<endl;
      if (_matchingType == EDITION){
	if (_mappingType == GLOBAL){
	  if (_mapping == GENERAL){
	    M = new MatchingGomde(Tree1,Tree2,*MCF,3);
	  }
	  else if (_mapping == DISTANCE){
	    M = new MatchingGomde(Tree1,Tree2,*MCF,3);
	  }
	  else if (_mapping == SIMILARITY){
	    M = new MatchingGomse(Tree1,Tree2,*MCF,3);
	  }
	  ...
	  if (_mapping == SIMILARITY){ 	  
	    M = new MatchingLomse(Tree1,Tree2,*MCF,3);
	  }
	  else{
	    cerr<<"Not yet Implemented"<<endl<<"General topological Matching"<<endl;
	    M = new Matching_O(Tree1,Tree2,*MCF);}
	...
	M = new Matching_O(Tree1,Tree2,*MCF);
      ...
      cout<<"quotiented"<<endl;
      if (_matchingType == EDITION){
	if (_mappingType == GLOBAL){
	  if (_mapping == GENERAL){
	    M = new MatchingGoqde(Tree1,Tree2,*MCF);
	  }
	  
	  else{
	    cerr<<"Not yet Implemented"<<endl<<"General topological Quotiented Matching"<<endl;
	    M = new MatchingGoqde(Tree1,Tree2,*MCF);
	  }
	}
  ...
  int mat_number = 0;
  int sub_number = 0;
  DistanceType sub_cost = M->getMatchingCost();
  
  DistanceType del_cost = M->getDeletionCost();
  
  DistanceType ins_cost = M->getInsertionCost();
  ...
  if (s->getSize())
    {
	  ...
	  S->append(tree_node1->getVertex(),tree_node2->getVertex(),scost);
        } while(s->next());
    }
...
// STANDARTIZATION
//---------------------------------------
void TreeMatch_O::standardization()
{
...
DistanceType TreeMatch_O::meanAbsoluteDifferenceComputation(int variable)
{
  int nb_tree = _distances.size();
