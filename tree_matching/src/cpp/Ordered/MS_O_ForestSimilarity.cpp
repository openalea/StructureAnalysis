...
//-----------------------------------------------------------------------------
void MS_O_Similarity::computeForestsDistances(int input_vertex,int reference_vertex){
  ...
  if(i>0 || j>0){  
	...
	if(L1[i1] == L1[i] && L2[j1] == L2[j]){
	  ...
	  DistanceType max = Max(dele)<<inse<<match;
	  ...
	  DistanceType dele = fDist[fNum][idx(L1[i],i1-1)][idx(L2[j],j1)] + delLocalNode(T1,i1);
	  DistanceType inse = fDist[fNum][idx(L1[i],i1)][idx(L2[j],j1-1)] +  insLocalNode (T2,j1);
	  DistanceType match = fDist[fNum][idx(L1[i],L1[i1]-1)][idx(L2[j],L2[j1]-1)] + fDist[fix(getKey1(i1),getKey2(j1))][idx(L1[i1],i1)][idx(L2[j1],j1)];
	  DistanceType max = Max(dele)<<inse<<match;
...
int MS_O_Similarity::fix(int a, int b){
  int sizeT2 = T2->getNbVertex(); 
  return a*sizeT2 +b;
}


int MS_O_Similarity::idx(int a, int b){
  return (a>b)?0:(b-a+1);
}
