...
#include "matching_ulcs.h"


  // -------------
  // Constructeur
  // -------------
Matching_U_LargestCommonSubTree::Matching_U_LargestCommonSubTree(TreeGraph& input,TreeGraph& reference,NodeCost& nodeDistance):
  Matching_U_MinCo(input,reference,nodeDistance)
{
  _modifiedDistances.make(*T1,*T2,nodeDistance);
}
  // -------------
  // Destructeur
  // -------------
Matching_U_LargestCommonSubTree::~Matching_U_LargestCommonSubTree()
{
}
  ...
  // ----------------------------------------------------------------------------------
DistanceType Matching_U_LargestCommonSubTree::distanceBetweenTree(int input_vertex,int reference_vertex)
{
...
//-----------------------------------
  switch (MTC)
    ...
    case 1 :{
      ...
      _modifiedDistances.putDBT(input_vertex,reference_vertex,
				getDBT(EMPTY_TREE,reference_vertex)+getDBT(input_vertex,EMPTY_TREE));
    };break;
    case 2 :{
    ...
    case 3 :{
      ...
      _InputRootTable.putNBC(input_vertex,reference_vertex,1);
      _ReferenceRootTable.putNBC(input_vertex,reference_vertex,1);
      _modifiedDistances.putDBT(input_vertex,reference_vertex,MIN);
    };break;
...
// forets
// -------------------------------
DistanceType Matching_U_LargestCommonSubTree::distanceBetweenForest(int input_vertex,int reference_vertex)
{
...
DistanceType Matching_U_LargestCommonSubTree::match()
{
  ...
  if ((!T1->isNull())&&(!T2->isNull()))
      ...
      for (int input_vertex=T1->getNbVertex()-1;input_vertex>=0;input_vertex--)
	{
	  ...
	  _modifiedDistances.openDistancesVector(input_vertex);
	  ...
	  for (int i=1;i<=T1->getNbChild(input_vertex);i++)
	    {
	      ...
	      _modifiedDistances.closeDistancesVector(T1->child(input_vertex,i));
...
int Matching_U_LargestCommonSubTree::getNBC(int input_vertex,int reference_vertex) const
{
  return(_nbInputTreeConnectedComponents.getNBC(input_vertex,reference_vertex));
}

int Matching_U_LargestCommonSubTree::getNBCRef(int input_vertex,int reference_vertex) const
{
