...
#include "local_matching.h"
  ...
  // Constructeur
  // -------------
LocalMatching::LocalMatching(TreeGraph& input,TreeGraph& reference,NodeCost& nodeDistance)
{
  T1=&input;
  T2=&reference;
  ND=&nodeDistance;
...
// ----------------------------------------------------------------------------------
DistanceType LocalMatching::distanceBetweenTree(int input_vertex,int reference_vertex)
{
...
// forets
// -------------------------------
DistanceType LocalMatching::distanceBetweenForest(int input_vertex,int reference_vertex)
{
  ...
  else
    {
	  ...
	  cost3 = 0.;
      ...
      else
        {
          ...
          cost3=_restrMapp.maxCostFlow(_restrMappList);
	  cout<<"cost 3 = "<<cost3<<endl;
...
void LocalMatching::getList(int input_vertex, int reference_vertex, Sequence* sequence)
{
  TreeList(input_vertex,reference_vertex,*sequence);
}

int LocalMatching::Lat(ChoiceList* L, int vertex)
{
...
int LocalMatching::getMaxInput()
{
  return (_maxInput);
}

int LocalMatching::getMaxRef()
{
  return (_maxRef);
}

void LocalMatching::TreeList(int input_vertex,int reference_vertex,Sequence& sequence)
{
...
void LocalMatching::ForestList(int input_vertex,int reference_vertex,Sequence& sequence)
{
  ChoiceList* L=_choices.getList(input_vertex,reference_vertex);
...
DistanceType LocalMatching::match()
{
...
// --------------------------------------------
DistanceType LocalMatching::getDistanceMatrix(int iv,int rv) const
{
  return(_distanceMatrix[iv][rv]);
}

DistanceType LocalMatching::getDBT(int input_vertex,int reference_vertex) const
{
...
DistanceType LocalMatching::getDBF(int input_vertex,int reference_vertex) const
{
  if ((input_vertex == -1) || (reference_vertex==-1))
    return (0.);
  else
...
int LocalMatching::M(int i_node,int r_node)
{
