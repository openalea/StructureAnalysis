...
EndSpaceFreeMatching::EndSpaceFreeMatching(TreeGraph& input,TreeGraph& reference,NodeCost& nodeDistance)
{
  T1=&input;
  T2=&reference;
  ND=&nodeDistance;
...
DistanceType EndSpaceFreeMatching::match()
{
...
DistanceType EndSpaceFreeMatching::match_begin_T2()
{
	const int size1 = T1->getNbVertex();
	const int size2 = T2->getNbVertex();
	DistanceType D=MAXDIST;
...
DistanceType EndSpaceFreeMatching::match_begin_end_T2()
{
	const int size1 = T1->getNbVertex();
	const int size2 = T2->getNbVertex();
	DistanceType D=MAXDIST;
